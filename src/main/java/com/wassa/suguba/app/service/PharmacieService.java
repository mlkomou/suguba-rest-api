...
public class PharmacieService {
    ...
    public Map<String, Object> updateStatut(UpdateStatut updateStatut) {
        try {
            Optional<Pharmacie> commandeOptional = pharmacieRepository.findById(updateStatut.getCommandeId());
            if (commandeOptional.isPresent()) {
                Pharmacie commande = commandeOptional.get();
                commande.setLastModifiedAt(LocalDateTime.now());
                commande.setStatut(updateStatut.getStatut());
                Pharmacie commandeSaced = pharmacieRepository.save(commande);
